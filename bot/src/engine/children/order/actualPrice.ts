 ...
 *
 * ⚠️ ВЛАДЕНИЕ (ADR-001 Вариант 3, B0-контракт 2026-06-24): расчёт цены живёт в **Core** (доменный
 * сервис ценообразования) на сервере, НЕ в канале/боте. FSM только фиксирует посчитанные значения,
 * бот только рендерит `price.*` из снапшота API. Поэтому этот модуль — НЕ продовый код бота, а
 * **исполняемая спецификация** алгоритма `Actual` для Core (@spitegod). На проде в боте не вызывается;
 * допустимо разве что как клиентская предпросмотр-валидация. См.
 * docs/integration/bot-domain-api-contract.md §5, docs/architecture-decision-variant3.md §5.
 *
