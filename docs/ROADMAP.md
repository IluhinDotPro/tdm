...
| Вопрос | Решение |
|---|---|
| Кодовая база нового бота | **Эволюция существующего** (заказчик: «эволюция понятнее»). Какой из двух — решаем после gap-анализа (Этап 5); предв. рекомендация — MultiBot (см. ниже). |
| Владелец FSM заказа | **Внешний — текущий бэкенд iBronevik** (`ibronevik.ru/taxi/c/.../api/v1`). Интегрируемся **под текущий API** (`b_state` + поля водителя `c_*`), но через абстрактный контракт `OrderGateway`. |
