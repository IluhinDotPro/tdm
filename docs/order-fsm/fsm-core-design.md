...
### 2.4 ⚠️ Развилка: владелец Driver Matching (главный архитектурный вопрос)
...
> ⭐ **Вопрос №1 — главная развилка (§2.4): владелец Driver Matching.** Подбор внутри Order FSM
> (Вариант A: `order_vote_waiting_*` = состояния Order) или отдельный Matching/Trip FSM (Вариант B)?
> 12 состояний [states.md](states.md) §1a читаются как A — подтвердить, что движок не выделяет
> отдельную Matching-машину при усложнении (re-matching, no_show), и являются ли Candidate/Offer
> отдельными сущностями движка. **Решается сверкой с движком, не на стороне бота.**
