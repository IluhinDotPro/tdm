...
- 🔴 **до MVP** (иначе процесс заказа зависает / некорректен): **timer worker** (#3) — без него `order_expire`/`order_no_show` в seed есть, но не срабатывают (T15/T16); **`availableActions` через Domain API**; **атомарность записи состояния** (`orders.status` ↔ `server_fsm_instances`).
...
- ❓ **Новый вопрос (no-show асимметрия):** `order_no_show` ведёт только из `order_vote_driver_assigned` (VOTE). Для DIRECT/OFFER (`order_driver_assigned` не приехал) терминала по таймауту в seed нет — только ручной cancel. Решить с Иваном/Валентином: нужен ли no-show/timeout для DIRECT/OFFER.
