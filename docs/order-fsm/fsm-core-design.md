...
| T14 | {created, waiting_*, *_assigned, arrived} → `order_cancelled` | 👤 `cancel` | состояние **до** `order_in_ride` ([business-rules.md](../domain/business-rules.md) §4.1) | терминальное; `outcome=cancelled`. ⚠️ **В seed нет `order_driver_arrived → order_cancelled`** (Иван) — **ТРЕБУЕТСЯ добавить** (cancel разрешён до BOARDING_VERIFICATION = `order_driver_arrived`) |
...
1. ✅ **Состояния** — 12 финальны для MVP, имена совпадают 1:1. Промежуточных нет (подтвердить по `taxi_order_fsm_seed.sql`).
2. ✅ **Переходы табличные.** Машинный перечень есть: `domains/taxi/fsm_spec.py` + `domains/taxi/sql/taxi_order_fsm_seed.sql` — **запрошены** для построчной сверки T1–T16.
...
- 🔴 **до MVP** (иначе процесс заказа зависает / некорректен): **timer worker** (#3), **`availableActions` через Domain API**, **атомарность записи состояния** (`orders.status` ↔ `server_fsm_instances`), плюс добавить T14 `arrived → cancelled`.
- 🟡 **после MVP** (компенсируется Action Layer): универсальный **guard registry**, полноценный **outbox/idempotency framework** (#6).
