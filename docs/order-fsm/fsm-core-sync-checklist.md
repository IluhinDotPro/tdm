...
|---|:---:|---|
| FSM instance (экземпляр на сущность) | ? | в дампе — состояния как строки; отдельная таблица инстансов = развилка A |
| Таблица переходов (`from→to+action`) | ✅ | `fsm_states`/`fsm_actions`/`fsm_transitions` в дампе vote_fsm |
| Auto-действия на переходе | ✅ | `fsm_actions` (states 104-106, actions 106-110) |
| Guard / условия на переходе | ? | есть ли условные переходы (наш guard-слой A1 — на стороне бота) |
| Timer subsystem (таймеры состояния) | ? | на бэкенде `b_max_waiting`; таймер-движок как механизм — выяснить (→ §5) |
| Event journal / журнал переходов | ? | нужен для дедупа и аналитики (= §11 вопрос 7) |
| Button states (UI-проекция действий) | ✅ | `button_states` в дампе — близко к нашему AvailableActions (B0) |
| Nested FSM (вложенные машины) | ? | ⭐ напрямую решает развилку B (Matching/Trip как под-FSM Order) |
| Parallel FSM (несколько активных машин на заказ) | ? | ⭐ нужен, если Order + Matching + Trip живут одновременно |
| Saga / orchestration (координация машин) | ? | нужно для re-matching (развилка C) и связки Order↔Matching |

**Вывод раздела (заполнить):** механизмы, которых в движке НЕТ и которые надо добавить под такси: ____________
...
- [ ] **Вариант B** — отдельный Matching FSM (+ Trip FSM), `order_vote_waiting_*` = проекция
  — реализуемость зависит от §0: nested/parallel FSM в движке (без них Вариант B = ручная оркестрация)
